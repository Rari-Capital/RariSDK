...
exports.get0xSwapOrders = void 0;
const ethers_1 = require("ethers");
const axios_1 = __importDefault(require("axios"));
const get0xSwapOrders = (inputTokenAddress, outputTokenAddress, maxInputAmountBN, maxMakerAssetFillAmountBN) => {
    return new Promise(function (resolve, reject) {
        return __awaiter(this, void 0, void 0, function* () {
            let decoded;
            try {
                decoded = yield axios_1.default.get("https://api.0x.org/swap/v0/quote?affiliateAddress=0x10dB6Bce3F2AE1589ec91A872213DAE59697967a&excludedSources=mStable&sellToken=" +
                    inputTokenAddress +
                    "&buyToken=" +
                    outputTokenAddress +
            ...
            decoded.orders.sort((a, b) => a.makerAssetAmount / (a.takerAssetAmount + a.takerFee) < b.makerAssetAmount / (b.takerAssetAmount + b.takerFee)
                ...
                : -1);
            let orders = [];
            let inputFilledAmountBN = ethers_1.constants.Zero;
            let takerAssetFilledAmountBN = ethers_1.constants.Zero;
            let makerAssetFilledAmountBN = ethers_1.constants.Zero;
            for (let i = 0; i < decoded.orders.length; i++) {
                if (decoded.orders[i].takerFee > 0 &&
                    ...
                    continue;
                let takerAssetAmountBN = ethers_1.BigNumber.from(decoded.orders[i].takerAssetAmount);
                let takerFeeBN = ethers_1.BigNumber.from(decoded.orders[i].takerFee);
                ...
                let makerAssetAmountBN = ethers_1.BigNumber.from(decoded.orders[i].makerAssetAmount);
                ...
                if (maxMakerAssetFillAmountBN !== undefined && maxMakerAssetFillAmountBN !== null) {
                    if (maxMakerAssetFillAmountBN.sub(makerAssetFilledAmountBN).lte(makerAssetAmountBN)) {
                        ...
                        while (makerAssetAmountBN.mul(orderInputFillAmountBN).div(orderInputAmountBN).lt(orderMakerAssetFillAmountBN)) {
                            ...
                            orderInputFillAmountBN = orderInputFillAmountBN.add(ethers_1.constants.One);
                            tries++;
                        }
                    }
                    else {
...
exports.get0xSwapOrders = get0xSwapOrders;

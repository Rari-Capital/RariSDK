    ...
    this.rgt = {
      ...
      distributions: {
        ...
        getCurrentApy: async function (blockNumber, tvl) {
          if (blockNumber === undefined && tvl === undefined) {
            ...
            var rgtDistributedPastHourPerUsd = rgtDistributedPastHour.mul(constants.WeiPerEther).div(tvl);
            var rgtDistributedPastHourPerUsdInUsd = rgtDistributedPastHourPerUsd
              .mul(await self.rgt.getExchangeRate())
              .div(constants.WeiPerEther);
            return BigNumber.from(
              Math.trunc(((1 + rgtDistributedPastHourPerUsdInUsd / 1e18) ** (24 * 365) - 1) * 1e18),
            );
          }
        },
        getCurrentApr: async function (blockNumber, tvl) {
          ...
          const rgtDistributedPastHour = self.rgt.distributions
            .getDistributedAtBlock(blockNumber)
            .sub(self.rgt.distributions.getDistributedAtBlock(blockNumber - 270));
          const rgtDistributedPastHourPerUsd = rgtDistributedPastHour.mul(constants.WeiPerEther).div(tvl);
      ...
      sushiSwapDistributions: {
        ...
        getDistributedAtBlock: function (blockNumber) {
          ...
          const blocks = blockNumber - startBlock;
          return self.rgt.sushiSwapDistributions.FINAL_RGT_DISTRIBUTION.mul(BigNumber.from(blocks)).div(
        ...
        getCurrentApy: async function (blockNumber, totalStakedUsd) {
          if (blockNumber === undefined && totalStakedUsd === undefined) {
            ...
            if (blockNumber - 270 < self.rgt.sushiSwapDistributions.DISTRIBUTION_START_BLOCK)
              blockNumber = self.rgt.sushiSwapDistributions.DISTRIBUTION_START_BLOCK + 270;
            ...
            const rgtDistributedPastHourPerUsd = rgtDistributedPastHour.mul(constants.WeiPerEther).div(totalStakedUsd);
            const rgtDistributedPastHourPerUsdInUsd = rgtDistributedPastHourPerUsd
              .mul(await self.rgt.getExchangeRate())
              .div(constants.WeiPerEther);
            return BigNumber.from(
        ...
        totalStaked: async function () {
          return BigNumber.from(await self.contracts.RariGovernanceTokenUniswapDistributor.totalStaked());
        },
        ...
        stakingBalanceOf: async function (account) {
          return BigNumber.from(await self.contracts.RariGovernanceTokenUniswapDistributor.stakingBalances(account));
        },
        ...
        stakedReservesOf: async function (account) {
          ...
          return {
            rgt: reservesPerLpToken.rgt.mul(stakingBalance).div(constants.WeiPerEther),
            eth: reservesPerLpToken.eth.mul(stakingBalance).div(constants.WeiPerEther),
          };
        },
        deposit: async function (amount, options) {
          ...
          const allowance = await slp.methods.allowance(
            options.from,
            self.contracts.RariGovernanceTokenUniswapDistributor.options.address,
          );
          amount = BigNumber.from(amount);
          if (amount.gt(allowance))
            await slp.methods.approve(self.contracts.RariGovernanceTokenUniswapDistributor.options.address, amount);
          await self.contracts.RariGovernanceTokenUniswapDistributor.methods.deposit(amount);
        },
        withdraw: async function (amount, options) {
          await self.contracts.RariGovernanceTokenUniswapDistributor.methods.withdraw(amount).send(options);
        },
        getUnclaimed: async function (account) {
          return BigNumber.from(await self.contracts.RariGovernanceTokenUniswapDistributor.getUnclaimedRgt(account));
        },
        claim: async function (amount, options) {
      ...
      vesting: {
        ...
        getClaimFee: function (timestamp: number): BigNumber {
          ...
          var privateVestingEndTimestamp =
            self.rgt.vesting.PRIVATE_VESTING_START_TIMESTAMP + self.rgt.vesting.PRIVATE_VESTING_PERIOD;

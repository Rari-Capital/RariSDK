...
export default class EthereumPool extends StablePool {
  ...
  constructor(web3, subpools, getAllTokens) {
    ...
    this.allocations.getRawPoolAllocations = async function () {
      ...
      for (const poolName of self.allocations.POOLS) allocationsByPool[poolName] = constants.Zero;
    ...
    this.apy.getCurrentRawApy = async function () {
      ...
      for (var i = 0; i < self.allocations.POOLS.length; i++)
        poolApyBNs[i] = await self.pools[self.allocations.POOLS[i]].getCurrencyApys();
      ...
      if (totalBalanceBN.isZero()) {
        ...
        for (let i = 0; i < factors.length; i++) if (factors[i][1].gt(maxApyBN)) maxApyBN = factors[i][1];
        return maxApyBN;
      }

      // If balance > 0, calculate the APY using the factors
      let apyBN = constants.Zero;
      for (let i = 0; i < factors.length; i++) apyBN = apyBN.add(factors[i][0].mul(factors[i][1]).div(totalBalanceBN));
      return apyBN;
    };
    ...
    this.deposits.validateDeposit = async function (currencyCode, amount, sender) {
      ...
      let accountBalanceBN = await (currencyCode == 'ETH'
      ...
      if (currencyCode === 'ETH') {
        ...
        try {
          var [orders, inputFilledAmountBN, protocolFee, takerAssetFilledAmountBN, makerAssetFilledAmountBN, gasPrice] =
            await get0xSwapOrders(allTokens[currencyCode].address, 'WETH', amount);
        } catch (err: any) {
        ...
        if (inputFilledAmountBN.lt(amount))
          throw new Error('Unable to find enough liquidity to exchange ' + currencyCode + ' to ETH before depositing.');
    ...
    this.deposits.deposit = async function (currencyCode, amount, minEthAmount, options) {
      ...
      if (currencyCode === 'ETH') {
        ...
        // Make sure we have enough ETH for the protocol fee
        var ethBalanceBN = BigNumber.from(await web3.eth.getBalance(options.from));
        if (BigNumber.from(protocolFee).gt(ethBalanceBN))
          throw new Error('ETH balance too low to cover 0x exchange protocol fee.');
        ...
        return [makerAssetFilledAmountBN, BigNumber.from(protocolFee), approvalReceipt, receipt];
      }
    };

    this.withdrawals.getMaxWithdrawalAmount = async function (currencyCode, senderEthBalance, sender) {
      var allTokens = await self.getAllTokens();
      if (currencyCode !== 'ETH' && !allTokens[currencyCode]) throw new Error('Invalid currency code!');
      if (!senderEthBalance || senderEthBalance.lte(constants.Zero)) return [constants.Zero];

      // Get user fund balance
      if (senderEthBalance === undefined)
        senderEthBalance = BigNumber.from(await self.contracts.RariFundManager.balanceOf(sender));
      ...
      try {
        var [orders, inputFilledAmountBN, protocolFee, takerAssetFilledAmountBN, makerAssetFilledAmountBN, gasPrice] =
          await get0xSwapOrders('WETH', allTokens[currencyCode].address, senderEthBalance);
      } catch (err: any) {
        throw new Error('Failed to get swap orders from 0x API: ' + err);
      }
    ...
    this.withdrawals.validateWithdrawal = async function (currencyCode, amount, sender) {
      var allTokens = await self.getAllTokens();
      if (currencyCode !== 'ETH' && !allTokens[currencyCode]) throw new Error('Invalid currency code!');
      if (!amount || amount.lte(constants.Zero)) throw new Error('Withdrawal amount must be greater than 0!');

      // Get user fund balance
      var accountBalance = BigNumber.from(await self.contracts.RariFundManager.methods.balanceOf(sender).call());
      ...
      if (currencyCode === 'ETH') {
        ...
        try {
          var [orders, inputFilledAmountBN, protocolFee, takerAssetFilledAmountBN, makerAssetFilledAmountBN, gasPrice] =
            await get0xSwapOrders('WETH', allTokens[currencyCode].address, amount);
        } catch (err: any) {
          throw new Error('Failed to get swap orders from 0x API: ' + err);
        }
        ...
        if (makerAssetFilledAmountBN.lt(amount))
          throw new Error('Unable to find enough liquidity to exchange withdrawn ETH to ' + currencyCode + '.');
    ...
    this.withdrawals.withdraw = async function (currencyCode, amount, maxEthAmount, options) {
      if (!options || !options.from) throw new Error('Options parameter not set or from address not set.');
      var allTokens = await self.getAllTokens();
      if (currencyCode !== 'ETH' && !allTokens[currencyCode]) throw new Error('Invalid currency code!');
      if (!amount || amount.lte(constants.Zero)) throw new Error('Withdrawal amount must be greater than 0!');
      ...
      if (currencyCode === 'ETH') {
        ...
        try {
          var receipt = await self.contracts.RariFundManager.methods.withdraw(amount).send(options);
        } catch (err: any) {
        ...
        // Check maxEthAmount
        if (typeof maxEthAmount !== 'undefined' && maxEthAmount !== null && inputFilledAmountBN.gt(maxEthAmount))
          return [inputFilledAmountBN];
    ...
    this.history.getPoolAllocationHistory = async function (fromBlock, toBlock, filter) {
      ...
      if (toBlock >= 11085000 && fromBlock <= 11819251)
        events = await self.legacyContracts['v1.0.0'].RariFundController.getPastEvents('PoolAllocation', {
      ...
      if (toBlock >= 11819251 && fromBlock <= 12904645)
        events = events.concat(
          await self.legacyContracts['v1.2.0'].RariFundController.getPastEvents('PoolAllocation', {
    ...
    this.history.getCurrencyExchangeHistory = async function (fromBlock, toBlock, filter) {
      var events = [];
      if (toBlock >= 11085000 && fromBlock <= 11819251)
        events = await self.legacyContracts['v1.0.0'].RariFundController.getPastEvents('CompToEthTrade', {
          fromBlock: Math.max(fromBlock, 11085000),
          toBlock: Math.min(toBlock, 11819251),
          filter,
        });
      if (toBlock >= 11819251 && fromBlock <= 12904645)
        events = events.concat(
          await self.legacyContracts['v1.2.0'].RariFundController.getPastEvents('CompToEthTrade', {
            fromBlock: Math.max(fromBlock, 11819251),
            toBlock: Math.min(toBlock, 12904645),
            filter,
          }),
    ...
    this.history.getPostWithdrawalExchangeHistory = async function (fromBlock, toBlock, filter) {
      return toBlock >= 11085000
        ? await self.contracts.RariFundProxy.getPastEvents('PostWithdrawalExchange', {
            fromBlock: Math.max(fromBlock, 20000001),
            toBlock,
            filter,
          })
        : [];
    ...
    this.history.getPoolTokenTransferHistory = async function (fromBlock, toBlock, filter) {
      return toBlock >= 11085000

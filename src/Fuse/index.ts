  ...
  constructor(web3Provider: JsonRpcProvider | Web3Provider) {
    ...
    this.deployPool = async function (
      ...
      const saltsHash = utils.solidityKeccak256(
        ['address', 'string', 'uint'],
        [accountToImpersonate, poolName, receipt.deployTransaction.blockNumber],
      );
    ...
    this.deployPriceOracle = async function (
      ...
      switch (model) {
        ...
        case 'UniswapTwapPriceOracle': // Uniswap V2 TWAPs
          deployArgs = [
            conf.rootOracle ? conf.rootOracle : Fuse.UNISWAP_TWAP_PRICE_ORACLE_ROOT_CONTRACT_ADDRESS,
            conf.uniswapV2Factory ? conf.uniswapV2Factory : '0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f',
        ...
        case 'UniswapTwapPriceOracleV2': // Uniswap V2 TWAPs
          ...
          priceOracleContract = new ContractFactory(
            this.oracleContracts['UniswapTwapPriceOracleV2'].abi,
            this.oracleContracts['UniswapTwapPriceOracleV2'].bin,
            this.provider.getSigner(),
          );
          deployedPriceOracle = await priceOracleContract.deploy(deployArgs, { ...options });
          break;
        case 'ChainlinkPriceOracleV2':
          ...
          deployArgs = [conf.admin ? conf.admin : options.from, conf.canAdminOverwrite ? true : false];
          deployedPriceOracle = await priceOracleContract.deploy(deployArgs, { ...options });
          break;
        case 'UniswapV3TwapPriceOracle':
          ...
          deployArgs = [
            conf.uniswapV3Factory ? conf.uniswapV3Factory : '0x1f98431c8ad98523631ae4a59f267346ea31f984',
            conf.feeTier,
        ...
        case 'UniswapV3TwapPriceOracleV2':
          ...
          priceOracleContract = new ContractFactory(
            this.oracleContracts['UniswapV3TwapPriceOracleV2'].abi,
            this.oracleContracts['UniswapV3TwapPriceOracleV2'].bin,
            this.provider.getSigner(),
          );
          deployArgs = [
            conf.uniswapV3Factory ? conf.uniswapV3Factory : '0x1f98431c8ad98523631ae4a59f267346ea31f984',
        ...
        case 'MasterPriceOracle':
          priceOracleContract = new ContractFactory(
            this.oracleContracts['MasterPriceOracle'].abi,
            this.oracleContracts['MasterPriceOracle'].bin,
            this.provider.getSigner(),
          );
          deployArgs = [
    ...
    this.deployComptroller = async function (
      ...
      await deployedComptroller._become(deployedUnitroller.options.address, { ...options });
    ...
    this.deployInterestRateModel = async function (model: string, conf: interestRateModelConf, options: any) {
    ...
    this.deployCToken = async function (
      ...
      return conf.underlying !== undefined &&
        ...
        ? await this.deployCErc20(
            ...
            options,
            bypassPriceFeedCheck,
            Fuse.CERC20_DELEGATE_CONTRACT_ADDRESS ? Fuse.CERC20_DELEGATE_CONTRACT_ADDRESS : undefined,
          )
    ...
    this.deployCEther = async function (
      ...
      if (supportMarket) {
        if (collateralFactor)
          await comptrollerContract._supportMarketAndSetCollateralFactor(
            deployedCEtherDelegator.options.address,
            collateralFactor,
            { ...options },
          );
        else await comptrollerContract._supportMarket(deployedCEtherDelegator.options.address, { ...options });
      }
    ...
    this.deployCErc20 = async function (
      ...
      deployedCErc20Delegator.options.jsonInterface = JSON.parse(
        this.compoundContracts['contracts/CErc20Delegate.sol:CErc20Delegate'].abi,
      );
          ...
          try {
            ...
            if (conf.underlying.toLowerCase() === Fuse.WETH_ADDRESS.toLowerCase()) {
              ...
              if (isUniswapAnchoredView) {
                ...
                await uniswapOrUniswapAnchoredViewContract.postPrices(['0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48'], {
                  ...options,
                });
              }
            } else {

...
export default class Fuse {
  ...
  constructor(web3Provider: JsonRpcProvider | Web3Provider, chainId: ChainID) {
    ...
    this.deployComptroller = async function (
      ...
      await deployedUnitroller._setPendingImplementation(
        deployedComptroller.options.address,
        { ...options }
      );

      // Comptroller becomes unitroller.
      await deployedComptroller._become(deployedUnitroller.address, {
        ...options,
      });
      ...
      if (liquidationIncentive)
        await deployedComptroller.methods._setLiquidationIncentive(
          liquidationIncentive,
          { ...options }
        );
      if (priceOracle)
    ...
    this.deployAsset = async function (
      ...
      reserveFactor: BigNumberish, // Amount of accrue interest that will go to the pool's reserves. Usually 0.1
    ...
    this.deployCToken = async function (
      ...
      if (reserveFactorBN.gt(constants.Zero) || adminFeeBN.gt(constants.Zero)) {
        const fuseFee =
          await this.contracts.FuseFeeDistributor.callStatic.interestFeeRate();
        if (
          reserveFactorBN
            .add(adminFeeBN)
    ...
    this.deployCEther = async function (
      ...
      var errorCode = await comptroller._deployMarket(
        "0x0000000000000000000000000000000000000000",
        constructorData,
        collateralFactor
      );
    ...
    this.deployCErc20 = async function (
      ...
      console.log("1: Inside deployCerc20:", {
        conf, collateralFactor, reserveFactor, adminFee, options, bypassPriceFeedCheck, implementationAddress
      })
      ...
      console.log("2: Inside deployCErc20", { constructorData, deployArgs, comptroller })
      ...
      try {
        ...
        console.log("3: Inside deployCErc20", { errorCode })
        if (errorCode != constants.Zero)
        throw (
          "Failed to deploy market with error code: " +
          Fuse.COMPTROLLER_ERROR_CODES[errorCode]
        ...
        throw (
          "Failed to deploy market with error: " +
          err
        );
      }
      ...
      console.log("4: Inside deployCErc20", { receipt })
      ...
      const byteCodeHash = utils.keccak256(
        "0x" + this.compoundContracts["contracts/Unitroller.sol:Unitroller"]
      );

      console.log("5: Inside deployCErc20", { saltsHash, byteCodeHash })
      ...
      console.log("6: Inside deployCErc20", { cErc20DelegatorAddress })
    ...
    this.identifyInterestRateModel = async function (
    ...
    this.getInterestRateModel = async function (
      assetAddress: string
